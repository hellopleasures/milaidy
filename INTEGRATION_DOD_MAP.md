...
|---|---|---|---|---|---|---|---|---|---|
| MW-01 | P0 | DX/Tooling | DX/Tooling | Coverage policy governance | ~~Coverage threshold drift~~ Verified: all docs aligned to 25/25/15/25; `docs/plugins/publish.md` 80% note clarified as plugin-author bar distinct from repo floor; CI enforces via `.github/workflows/test.yml` `bun run test:coverage` | One canonical threshold decision; docs and `vitest.config.ts` match; CI fails on non-compliance | `bun run test:coverage` | `vitest.config.ts`, `AGENTS.md`, `CONTRIBUTING.md`, `docs/guides/contribution-guide.md`, `.github/workflows/agent-review.yml` | False confidence and inconsistent review standards |
| MW-02 | P0 | Blockchain | Blockchain | Wallet/registry/drop | ~~Missing direct tests~~ Resolved: 66 service-level + 8 route-level tests cover timeout, nonce/retry, and failure mapping | Add deterministic unit tests for tx timeout, nonce/retry behavior, and route->service failure mapping | `bunx vitest run src/api/tx-service.test.ts src/api/registry-service.test.ts src/api/drop-service.test.ts src/api/onchain-routes.test.ts` | `src/api/tx-service.test.ts`, `src/api/registry-service.test.ts`, `src/api/drop-service.test.ts`, `src/api/onchain-routes.test.ts` | On-chain failures/regressions can ship undetected |
...
| MW-04 | P0 | Security | Security | Websocket/auth surfaces | ~~Incomplete auth coverage~~ Resolved: 26 auth gate tests cover all sensitive mutation endpoints (config, secrets, connectors CRUD, MCP CRUD, wallet generate/config/import/export, agent restart, terminal run) | Add/extend auth tests for config mutation, secrets update, connector mutation, MCP mutation | `bunx vitest run src/api/server.config-secrets-auth.test.ts src/api/server.websocket-auth.test.ts src/api/server.mcp-config-validation.test.ts src/api/server.wallet-export-auth.test.ts` | `src/api/server.*.test.ts` | Unauthorized access vectors |
...
| MW-09 | P1 | DX/Tooling | DX/Tooling | Observability baseline | ~~No standardized metrics/traces for integration boundaries~~ Resolved: 78 observability tests across 8 files enforce integration_boundary_v1 contract (success/failure/latency) for all 4 boundaries with 15 production span operations | Define minimal metrics contract (success/failure/latency) for cloud, wallet, marketplace, and MCP boundaries | `bun run typecheck && bunx vitest run src/diagnostics/integration-observability.test.ts src/api/cloud-routes.observability.test.ts src/api/wallet-routes.observability.test.ts src/api/tx-service.observability.test.ts src/api/twitter-verify.observability.test.ts src/services/skill-marketplace.observability.test.ts src/services/mcp-marketplace.observability.test.ts src/services/registry-client-network.observability.test.ts` | `src/diagnostics/integration-observability.ts`, `src/diagnostics/integration-observability.test.ts`, `src/api/*.observability.test.ts`, `src/services/*.observability.test.ts` | Hard incident triage and SLO blind spots |
...
| MW-11 | P2 | Docs | Docs | Docs completeness | Verified — 6 runbook files cover all integration boundaries: registry (setup/failure/verify), drop (on-chain/whitelist/state), skills (catalog/marketplace/loading), connectors (12+ platforms), CUA (screenshot/vision/sandbox), skills-api (search/install/load) | Add setup + failure-mode + verification sections per integration boundary | `bun run docs:build` | `docs/guides/registry.md`, `docs/plugins/skills.md`, `docs/guides/connectors.md`, `docs/plugin-registry/computeruse.md`, `docs/rest/drop.md`, `docs/rest/skills.md` | Slower onboarding and misconfigurations |
